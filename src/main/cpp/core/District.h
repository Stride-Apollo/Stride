...
public:
	/// Constructor
	District(string district_name, uint influence_size, double influence_speed, double influence_minimum,
	...
	void addFacility(string facility_name) {
		if (!hasFacility(facility_name)) {
			m_transportations_facilities.push_back(
					make_pair(facility_name, Influence(m_influence_size, m_influence_speed, m_influence_minimum)));
		}
	}
