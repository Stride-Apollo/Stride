...
struct TravelData {
	...
	TravelData(vector<Simulator::TravellerType> travellers, uint amount, uint days, const string& destination_sim,
			   const string& destination_district, const string& destination_facility) :
			m_amount(amount), m_days(days), m_destination_simulator(destination_sim),
			m_destination_district(destination_district), m_destination_facility(destination_facility),
			m_travellers(travellers) {};
