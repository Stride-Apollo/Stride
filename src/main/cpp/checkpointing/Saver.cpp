...
Saver::Saver(const char* filename, ptree pt_config, int frequency): m_filename(filename), m_frequency(frequency), m_pt_config(pt_config) {
	try {
		...
		configData[0].generate_person_file = generate == 1 ? true : false; // TODO This definitely needs checking cuz I dont know if it works
		...
		configData[0].log_level = pt_config.get<std::string>("run.log_level").c_str(); // TODO This is a string in the config?
