...
namespace stride {
ptree Loader::get_config(unsigned int num_threads) {
	try {
		Exception::dontPrint();
		...
		delete dataset;
		std::cout << "Started building\n";
		std::shared_ptr<Simulator> sim = SimulatorBuilder::build(m_pt_config, num_threads, m_track_index_case);
